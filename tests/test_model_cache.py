...
class ModelCacheTests(unittest.TestCase):
    def test_model_cache_follows_hf_home(self):
        with tempfile.TemporaryDirectory() as directory:
            with patch.dict(os.environ, {"HF_HOME": directory, "HF_HUB_CACHE": ""}, clear=False):
    ...
    def test_download_uses_the_same_hub_cache_that_status_checks(self):
        spec = MODEL_CATALOG[ModelPreset.FAST]
        calls: list[tuple[str, str]] = []

        def fake_snapshot_download(*, repo_id: str, cache_dir: str) -> str:
            calls.append((repo_id, cache_dir))
            snapshot = Path(cache_dir) / ("models--" + repo_id.replace("/", "--")) / "snapshots" / "abc123"
            snapshot.mkdir(parents=True)
            (snapshot / "config.json").write_text("{}", encoding="utf-8")
            return str(snapshot)
        ...
        with tempfile.TemporaryDirectory() as directory:
            with patch.dict(os.environ, {"HF_HOME": directory, "HF_HUB_CACHE": ""}, clear=False):
                with patch.dict(sys.modules, {"huggingface_hub": fake_hub}):
                    download_model(spec)
                self.assertEqual([(spec.repository, str(Path(directory) / "hub"))], calls)
                self.assertTrue(model_is_installed(spec))
